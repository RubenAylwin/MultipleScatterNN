...
elseif strcmp(fe.opr,'n23*[psi]')
    ...
    m    = size(nrm,1);
    M{1} = 0* uqm;
    M{2} = spdiags(nrm(:,3),0,m,m) * uqm;
    M{3} = spdiags(-nrm(:,2),0,m,m) * uqm;
...
%%% NORMALS * DQM (j)
elseif strcmp(fe.opr(1:end-1),'n23*[psi]')
    % Component
    j = str2double(fe.opr(end));
    ...
    elseif (j==2)
      ...
      M = spdiags(N(:,3),0,m,m) * uqm;
    else

      M  = -spdiags(N(:,2),0,m,m) * uqm;
    end
%%% QUADRATURE * DQM (j)
