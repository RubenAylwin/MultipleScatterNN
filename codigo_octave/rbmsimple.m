...
usablespacegrid = 0.8*hgrid;

rmin = 0.7*usablespacegrid/2;
rmax = 0.9*usablespacegrid/2;

dmin = 2*(hgrid-usablespacegrid);
dmax =  2*LBox;

cn = rmin*0.25*(1./(1:Nper).^2)';
...
for nn=1:Nsnapshots
    ...
    Y = halton_value(nn-1,4*Nper+4)-0.5;

    rx =@(t) rX0(t,Y(4),Y(5:Nper+4),Y(Nper+5:2*Nper+4),Y(2));
    ry =@(t) rY0(t,Y(4),Y(2*Nper+5:3*Nper+4),Y(3*Nper+5:4*Nper+4),Y(3));


    meshb = meshClosedBoundary(N,rx,ry);
...
##
##
##    meshbii = meshClosedBoundary(N,rxii,ryii);
##
##    gammaii = dom(meshbii,3);
##    phiii = fem(meshbii, 'P0');
##
##    plot(rxii(ts),ryii(ts),colors{ii})
##
##
##
##    for jj=1:ii
...
##
##
##        meshbjj = meshClosedBoundary(N,rxjj,ryjj);
##
##        gammajj = dom(meshbjj,3);
##        phijj = fem(meshbjj, 'P0');
##
##        ind = jj+ii*(ii-1)/2;
##
##        mat{ind} = -0.5/pi*integral(gammajj,gammaii,phijj,Gxy,phiii,1e-6) -...
##        0.5/pi*regularize(gammajj,gammaii,phijj,'[log(r)]',phiii);
##
...
##  end
##
##    rhs((1:Ndof)+(ii-1)*Ndof) = integral(gamma,phi,g);
##
...
##
##  Y = halton_value(nn-1,8*Nper+3)-0.5;
##
##  rx1 =@(t) rX1(t,Y(1),Y(2),Y(4:Nper+3),Y(Nper+4:2*Nper+3));
##  ry1 =@(t) rY1(t,Y(2),Y(2*Nper+4:3*Nper+3),Y(3*Nper+4:4*Nper+3));
##
##  rx0 =@(t) rX0(t,Y(3),Y(4*Nper+4:5*Nper+3),Y(5*Nper+4:6*Nper+3),0);
##  ry0 =@(t) rY0(t,Y(3),Y(6*Nper+4:7*Nper+3),Y(7*Nper+4:end),0);
##
