...
methods
    ...
    function mesh = msh(varargin)
        ...
        if((size(mesh.vtx,2) ==3)&&(size(mesh.elt,2) ==3))
          ...
          mesh.maps = cell(size(mesh.elt,1),1);
          mapsI = cell(size(mesh.elt,1),1);

          for ee = 1:size(mesh.elt,1)

            v1 = mesh.elt(ee,1);
            v2 = mesh.elt(ee,2);
            v3 = mesh.elt(ee,3);

            mesh.maps{ee} = [mesh.vtx(v2,:)-mesh.vtx(v1,:);...
                         mesh.vtx(v3,:)-mesh.vtx(v1,:)].';
...
##    function [elet,barcords]  = point2elt(mesh,X)
##
##      elet = 0;
##
##      x = reshape(X,3,1);
...
##      for ee =1:size(mesh.elt,1)
##
##          barcords = mesh.maps{ee}\(x-mesh.vtx(mesh.elt(ee,1),:).');
##
##          if((sum(barcords) <= 1) && (all(barcords>=0)))
##            elet = ee;
##            barcords = barcords.';
##            return;
##          end
...
##    end
##
##    function [elts,cords] = point2elts(mesh,X)
##
##      npoints = size(X,1);
##
##      elts = zeros(npoints);
##      cords = zeros(npoints,2);
##
##      for ii = 1:npoints
##
##        [elts(ii),cords(ii,:)] = point2elt(mesh,X(ii,:));
##
##      end
##
##
...
##    end
    ...
    function [elts,cords] = point2elts(mesh,X)
      ...
      [elts,cords] = tsearchn2 (mesh.vtx, mesh.elt, X);



  end

   function [elts,cords] = point2elts2d(mesh,X)


      [elts,cords] = tsearchn2d(mesh.vtx, mesh.elt, X);
